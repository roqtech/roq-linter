...
module.exports = {
  ...
  create(context) {
    ...
    const isIndexFile = file.name === 'index.tsx' || file.name === 'index.ts';
    const ruleExecutionRequired = (isComponent || isPartial) && !isIndexFile;
      ...
      reportingObj.Program = (node) => {
        ...
        if (file.nameWithoutExt.split('.')[0] !== rawDirName) {
          context.report({
            ...
            data: {
              expectedFileName: `${rawDirName}${isComponent ? '.component' : 'partial'}.ts`,
            },
          });
        }
      };
