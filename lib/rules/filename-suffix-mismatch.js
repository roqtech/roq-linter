  ...
  create(context) {
    ...
    if (!isIgnored && !isIndexFile) {
      ...
      */
      ...
      const isLayout = resourceRegex.layout.some((e) => e.test(dirPath));
      ...
      if (isComponent) {
        ...
        if (mismatchType === 2) { // we can also expect styles files
          ...
          const partialSuffixCheck = checkSuffixMismatch(file, 'partial');
         if (!stylesSuffixCheck.mismatchType || !partialSuffixCheck.mismatchType) {
            mismatchType = 0;
          } else if (stylesSuffixCheck.mismatchType
            || partialSuffixCheck.mismatchType) {
            mismatchType = 1;
            expectedFileName = [...stylesSuffixCheck.expectedFileName, ...(stylesSuffixCheck.expectedFileName.map((e) => e.replace('styles', 'component'))), ...(stylesSuffixCheck.expectedFileName.map((e) => e.replace('styles', 'partial')))];
          }
