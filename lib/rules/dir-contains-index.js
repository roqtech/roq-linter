...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://app.archbee.io/public/EpeZApNOPw_vb0lzacxnR/s_a_-dir-contains-index',
    },
    schema: [], // no options
  },
  create(context) {
    ...
    const isIgnored = ignoredPaths.some((e) => e.test(dirPath)) || (file.name.includes('[') || dirPath.split(sep).slice(-1)[0].includes('[')); // Ignoring dynamic pages as well
    if (!isIgnored) {
