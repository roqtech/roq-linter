...
module.exports = {
  ...
  create(context) {
    ...
    const isComponentsDirectoryAtExpectedLocation = [
      new RegExp(`^${[frontendBasePattern, 'modules', allowedNamingPattern, 'components'].join(escapedSep)}`),
      new RegExp(`^${[pathPatterns.frontend.layouts, allowedNamingPattern, 'components'].join(escapedSep)}`),
      new RegExp(`^${[pathPatterns.frontend.views, allowedNamingPattern, 'components'].join(escapedSep)}`),
