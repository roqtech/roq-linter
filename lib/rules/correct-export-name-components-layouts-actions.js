...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      invalidExportedComponentInterfaceSuffix: 'Exported interfaces from components/partials should be suffixed with either (Interface/Props)',
    },
    schema: [],
  },
  create(context) {
    ...
    const isPartial = resourceRegex.partial.some((e) => e.test(dirPath)) && !file.nameWithoutExt.endsWith('.styles'); // styles files do not need to abide by this
    const isLayout = resourceRegex.layout.some((e) => e.test(dirPath)) && !file.nameWithoutExt.endsWith('.styles'); // styles files do not need to abide by this
    ...
    const ruleExecutionRequired = (isComponent || isLayout || isSliceOrAction || isPartial) && !isIndexFile;
        ...
        if (node.declaration && (node.declaration.id || node.declaration.declarations[0].id)) {
          switch (node.declaration.type) {
            ...
            case 'TSInterfaceDeclaration': {
              const { name, loc } = node.declaration.id;
              const componentInterfaceIdentifyingSuffixes = ['Interface', 'Classes', 'Props'];

              const interfaceExportCorrectPrefix = name.startsWith(pascalCased);

              const interfaceExportCorrectSuffix = (isComponent || isPartial) ? (componentInterfaceIdentifyingSuffixes.some((suffix) => name.endsWith(suffix))) : name.endsWith('Interface');

              if (!interfaceExportCorrectPrefix) {
                invalidDeclarationError(context, loc, 'invalidExportNamePrefix');
              }
              if (!interfaceExportCorrectSuffix) {
                invalidDeclarationError(context, loc, isComponent ? 'invalidExportedComponentInterfaceSuffix' : 'invalidExportNameSuffix');
              }
              break;
            }
